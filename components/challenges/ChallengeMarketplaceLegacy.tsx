...
import { CandidateDialogueCapacity, CareerMapGraphModel, DiscoveryFilterSource, Job, JobSearchFilters, MicroJobCollaborationMode, MicroJobLongTermPotential, SearchLanguageCode, SearchMode, SupportedCountryCode, UserProfile } from '../../types';
...
import { EmptyState, FilterChip, MetricTile, SurfaceCard, Toolbar, cn } from '../ui/primitives';
...
type DiscoveryMode = 'all' | 'micro_jobs';
...
const ChallengeMarketplace: React.FC<ChallengeMarketplaceProps> = ({
  ...
  const microJobsOnly = discoveryMode === 'micro_jobs';
  ...
  const ForestBackdrop: React.FC<{ className?: string }> = ({ className }) => (
    <div className={cn("pointer-events-none absolute inset-0 -z-10 overflow-hidden", className)}>
      <div
        className={cn(
          "absolute -top-28 -left-24 h-[22rem] w-[22rem] blur-3xl",
          "rounded-[48%_52%_55%_45%/55%_45%_55%_45%]",
          "bg-[radial-gradient(circle_at_30%_28%,rgba(var(--accent-green-rgb),0.34),transparent_62%)]",
          "dark:opacity-80"
        )}
      />
      <div
        className={cn(
          "absolute -bottom-28 -right-24 h-[24rem] w-[24rem] blur-3xl",
          "rounded-[55%_45%_45%_55%/45%_55%_45%_55%]",
          "bg-[radial-gradient(circle_at_65%_70%,rgba(var(--accent-sky-rgb),0.28),transparent_60%)]",
          "dark:opacity-80"
        )}
      />
      <div
        className={cn(
          "absolute top-[18%] right-[12%] h-44 w-44 blur-2xl",
          "rounded-[60%_40%_70%_30%/45%_55%_45%_55%]",
          "bg-[radial-gradient(circle_at_50%_50%,rgba(var(--accent-rgb),0.22),transparent_62%)]",
          "dark:opacity-70"
        )}
      />
    </div>
  ...
  // Legacy sidebar used a "swipe vs list" desktop split. Control Center is now the primary desktop UX,
  // so keep legacy on desktop in list mode to avoid a hidden main column when the sidebar is disabled.
  useEffect(() => {
    if (!isLgViewport) return;
    setMobileViewMode('list');
  }, [isLgViewport]);
  // Desktop/tablet list view should not "navigate" (scroll to top / route-like changes).
  // Keep local selection for lg+ and show details in the right column.
  const [desktopSelectedJobId, setDesktopSelectedJobId] = useState<string | null>(null);
  // Map view should also avoid global navigation (AppSceneRouter swaps to ChallengeFocusView on selectedJobId).
  // Keep local selection and show detail as a modal above the map.
  const [mapSelectedJobId, setMapSelectedJobId] = useState<string | null>(null);
  ...
  const [viewMode, setViewMode] = useState<'list' | 'map'>('list'); // Default to list view
  ...
  const careerMapCacheRef = useRef<Map<string, CareerMapGraphModel>>(new Map());
  const [filtersDrawerOpen, setFiltersDrawerOpen] = useState(false);
  const [careerMapMode, setCareerMapMode] = useState<'hierarchy' | 'taxonomy'>('hierarchy');

  const copy = ({
    cs: {
      eyebrow: 'Mapa práce',
      title: 'Najdi týmy a problémy, kde můžeš něco zlepšit',
      body: 'Feed čte práci přes týmové situace, realitu života a první smysluplný krok, ne jen přes název pozice.',
      ...
      cockpitTitle: 'Rozhodovací vrstva',
      cockpitBody: 'Nastavení převádí tvoji situaci do použitelnějšího čtení trhu. Výsledkem nemá být šum, ale role, které dávají smysl právě teď.',
      firstContactTitle: 'První kontakt jako dialog',
      firstContactBody: 'Kde to jde, nezačínáš slepým CV, ale krátkou odpovědí na situaci, kterou tým opravdu řeší.',
      lifeFiltersTitle: 'Filtry podle skutečného života',
      lifeFiltersBody: 'Dojezd, hranice, jazyk, režim práce nebo rytmus rodiny. Hledání se má ohnout podle života, ne naopak.',
      laneBadge: lane === 'imports' ? 'Čtení importovaného trhu' : 'Čtení nativních výzev',
      laneBody:
        lane === 'imports'
          ? 'Širší trh, který JobShaman převypráví přes problém, realitu práce a to, co si máš ověřit.'
          : 'Nativní role, kde je první kontakt blíž skutečné práci a ne jen odeslání dokumentu.',
      ...
      matchesNow: 'Dává smysl právě teď',
      setupEmpty: 'Přidejte filtry nebo preference a přehled se začne skládat podle vaší reality.',
      setupSection: 'Nejblíž tomu, kde můžeš pomoct',
      setupSectionBody: 'Role, které nejlépe sedí tvému směru, cílové roli a reálným podmínkám života.',
      ...
      manualSection: 'Role podle toho, co právě zkoumáš',
      manualSectionBody: 'Výsledky podle zadaných slov a aktuálně zapnutých reality filtrů.',
      ...
      fitNote: 'Feed zvýrazní role, které mají největší šanci dávat smysl profesně i lidsky.',
      challengeLabel: 'Co bude potřeba zvládnout',
      riskLabel: 'Na co si dát pozor',
      openCard: 'Prozkoumat roli',
    ...
    en: {
      eyebrow: 'Work map',
      title: 'Find teams and problems where you can improve something real',
      body: 'The feed reads work through team situations, life reality, and meaningful next steps, not just through job titles.',
      ...
      cockpitTitle: 'Decision layer',
      cockpitBody: 'Your setup turns life context into a clearer market read. The result should be a usable direction, not a noisy feed.',
      firstContactTitle: 'First contact as dialogue',
      firstContactBody: 'Where possible, the first move is not a blind resume drop but a short reply to a real team situation.',
      lifeFiltersTitle: 'Filters shaped by lived reality',
      lifeFiltersBody: 'Commute, borders, language, work mode, and family logistics all matter. Search should bend around life, not the other way around.',
      laneBadge: lane === 'imports' ? 'Imported market read' : 'Native challenge view',
      laneBody:
        lane === 'imports'
          ? 'A broader market that JobShaman reframes through the problem, work reality, and what you should verify before leaving.'
          : 'Native roles where first contact is closer to the real work than to sending documents.',
      ...
      matchesNow: 'Makes sense now',
      setupEmpty: 'Add filters or preferences and the feed will start shaping around your reality.',
      setupSection: 'Closest to where you can help',
      setupSectionBody: 'Roles that best match your direction, target role, and real-life constraints.',
      ...
      manualSection: 'Roles based on what you are exploring',
      manualSectionBody: 'Results based on your search terms and currently active life filters.',
      ...
      viewModeList: 'List',
      viewModeMap: 'Map',
      emptyTitle: 'Nothing matches well right now',
      ...
      fitNote: 'The feed highlights roles with the strongest chance of real professional and human fit.',
      challengeLabel: 'Challenge',
      riskLabel: 'Risk',
      openCard: 'Explore role',
      slotsTitle: 'Dialogue slots',
      slotsBody: 'Each reply opens a limited number of active dialogues, so the system does not turn into an endless funnel with no response.',
  ...
  const microJobDiscoveryCopy = {
    ...
    laneReadMode: getLocaleLabel({
      cs: 'Malý problém, který stojí za vyřešení',
      ...
      en: 'Small problem worth solving'
    }, language),
  };
  ...
  */
  ...
  const careerMapCenterNode = useMemo(() => {
    const role = String(effectiveCandidateIntent.targetRole || '').trim();
    const domain = String(effectiveCandidateIntent.primaryDomain || '').trim();
    if (role) {
      return {
        title: role,
        subtitle: domain ? `Domain: ${domain.replace(/[_-]+/g, ' ')}` : null,
      };
    }
    if (domain) {
      return {
        title: domain.replace(/[_-]+/g, ' ').replace(/\b\w/g, (m) => m.toUpperCase()),
        subtitle: isCsLike ? 'Váš obor (z profilu)' : 'Your domain (from profile)',
      };
    }
    return {
      title: isCsLike ? 'Vy' : 'You',
      subtitle: isCsLike ? 'Začneme od oborů a doporučených firem.' : 'Start from domains and recommended companies.',
    };
  }, [effectiveCandidateIntent.primaryDomain, effectiveCandidateIntent.targetRole, isCsLike]);
  ...
  const jobsInLane = useMemo(() => {
    ...
    const byCommuteReality = (items: Job[]) =>
      items.filter((job) => {
        ...
        if (distanceKm == null || distanceKm > effectiveCommuteRadiusKm) {
          return job.listingKind === 'imported' && computedDistance == null;
        }
    ...
    const mergePools = (primaryPool: Job[], secondaryPool: Job[]) => {
      const seen = new Set(primaryPool.map((job) => job.id));
      return [
        ...primaryPool,
        ...secondaryPool.filter((job) => !seen.has(job.id)),
      ];
    };
    ...
    if (nativeScopedMatches.length < MIN_NATIVE_CHALLENGE_POOL) {
      return mergePools(nativeScopedMatches, importedScopedMatches);
    }
    if (!usePersonalSetup) {
      return mergePools(nativeScopedMatches, importedScopedMatches);
    }
  ...
  const mapEnabled = isMapMode || viewMode === 'map';
  const listLoadMoreSentinelRef = useRef<HTMLDivElement | null>(null);

  useEffect(() => {
    if (!isLgViewport) return;
    setViewMode(isMapMode ? 'map' : 'list');
  }, [isLgViewport, isMapMode]);

  useEffect(() => {
    if (mobileViewMode === 'swipe' && !isXlViewport) return;
  ...
  }, [hasMore, loading, loadingMore, loadMoreJobs, mobileViewMode, isXlViewport]);

  useEffect(() => {
    const shouldLock = mapEnabled || filtersDrawerOpen;
  ...
  }, [filtersDrawerOpen, mapEnabled]);

  const careerMapJobPool = useMemo(() => {
    const pool = prioritizedJobsInLane.slice(0, 60);
    if (selectedJobId && !pool.some((job) => job.id === selectedJobId)) {
      const selected = prioritizedJobsInLane.find((job) => job.id === selectedJobId);
      if (selected) pool.unshift(selected);
    }
    return pool;
  }, [prioritizedJobsInLane, selectedJobId]);
  const careerMapJobsKey = useMemo(() => careerMapJobPool.map((job) => job.id).join('|'), [careerMapJobPool]);

  useEffect(() => {
    if (!mapEnabled) return;
    if (mapSelectedJobId) return;
    if (selectedJobId) {
      setMapSelectedJobId(selectedJobId);
      return;
    }
    if (careerMapJobPool.length > 0) {
      setMapSelectedJobId(careerMapJobPool[0].id);
    }
  }, [careerMapJobPool, mapEnabled, mapSelectedJobId, selectedJobId]);

  useEffect(() => {
    if (typeof window === 'undefined') return;
    const onOpenFilters = () => setFiltersDrawerOpen(true);
    window.addEventListener('jobshaman:openDiscoveryFilters', onOpenFilters as EventListener);
    return () => window.removeEventListener('jobshaman:openDiscoveryFilters', onOpenFilters as EventListener);
  }, []);

  useEffect(() => {
    if (!mapEnabled) return;
    if (careerMapJobPool.length === 0) return;

    const cached = careerMapCacheRef.current.get(careerMapJobsKey);
    if (cached) {
    ...
    setCareerMapError(null);
    buildCareerMapGraphModel(careerMapJobPool)
      .then((graph) => {
        if (cancelled) return;
        careerMapCacheRef.current.set(careerMapJobsKey, graph);
  ...
  }, [careerMapJobPool, careerMapJobsKey, mapEnabled]);

  useEffect(() => {
    if (!mapEnabled) return;
    // When leaving/entering map, keep a predictable default.
    setCareerMapMode('hierarchy');
  }, [mapEnabled]);
    ...
    if (language === 'de' || language === 'at') {
      return [
        ...
        },
        {
          ...
          premiumActiveNote: 'Die vollständige JCFPM-Interpretation ist für dich freigeschaltet. Der praktische Effekt liegt in klarerer Begründung der Empfehlungen, nicht mehr Interface-Lärm.'
        }
      ];
    }
  ...
  useEffect(() => {
    if (!isLgViewport) return;
    if (desktopSelectedJobId) return;
    if (!selectedJobId) return;
    setDesktopSelectedJobId(selectedJobId);
  }, [desktopSelectedJobId, isLgViewport, selectedJobId]);

  const effectiveSelectedJobId = useMemo(() => {
    return isLgViewport ? desktopSelectedJobId : selectedJobId;
  }, [desktopSelectedJobId, isLgViewport, selectedJobId]);

  const selectedFeedJob = useMemo(() => {
    if (!effectiveSelectedJobId) return null;
    return (
      prioritizedJobsInLane.find((job) => job.id === effectiveSelectedJobId) ||
      jobsInLane.find((job) => job.id === effectiveSelectedJobId) ||
      jobs.find((job) => job.id === effectiveSelectedJobId) ||
      null
    );
  }, [effectiveSelectedJobId, jobs, jobsInLane, prioritizedJobsInLane]);

  const renderDiscoveryFiltersContent = () => (
    <div className="space-y-5 [&_.app-filter-chip]:!rounded-[0.95rem]">
  ...
  const FiltersModal = () => (
    <AnimatePresence>
      {filtersDrawerOpen ? (
        <motion.div className="fixed inset-0 z-[85]">
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          <motion.div
            ...
            className={cn(
              "absolute top-[calc(var(--app-header-height)+12px)] bottom-4",
              isLgViewport
                ? "left-24 w-[420px] max-w-[calc(100vw-96px)]"
                : "left-1/2 w-[min(560px,calc(100vw-24px))] -translate-x-1/2"
            )}
          >
            <SurfaceCard className="h-full overflow-hidden border-[rgba(var(--accent-rgb),0.14)] shadow-[var(--shadow-overlay)]">
              <div className="flex items-center justify-between gap-3 border-b border-[var(--border-subtle)] px-4 py-3">
                <div className="text-sm font-semibold text-[var(--text-strong)]">{copy.filters}</div>
                <button type="button" className="app-button-secondary !px-3 !py-2" onClick={() => setFiltersDrawerOpen(false)}>
                  {isCsLike ? 'Zavřít' : 'Close'}
                </button>
              </div>
              <div className="h-[calc(100%-52px)] overflow-y-auto p-4">
                {renderDiscoveryFiltersContent()}
              </div>
            </SurfaceCard>
          </motion.div>
        </motion.div>
  ...
  if (mapEnabled) {
    return (
      <section style={forestAccentStyle} className="relative">
        <div className="fixed inset-0 z-40">
          <div
            aria-hidden
            className={cn(
              'absolute inset-0',
              'bg-[linear-gradient(135deg,rgba(248,252,249,0.98),rgba(240,250,244,0.98)_46%,rgba(236,248,242,0.98))]',
              'dark:bg-[linear-gradient(135deg,rgba(8,20,12,0.92),rgba(7,16,12,0.94)_48%,rgba(4,12,9,0.96))]'
            )}
          />
          <ForestBackdrop className="z-0 opacity-95" />
          <div className={cn(
            "absolute right-4 top-[calc(var(--app-header-height)+16px)] z-[60] flex items-center gap-2",
            !isLgViewport ? "" : "right-6"
          )}>
            <div className="flex items-center gap-1.5 rounded-2xl border border-[var(--border-subtle)] bg-white/70 p-1 shadow-[var(--shadow-soft)] backdrop-blur dark:bg-[rgba(15,23,42,0.55)]">
              <button
                type="button"
                className={cn(
                  "px-3 py-2 text-sm font-semibold rounded-xl transition",
                  careerMapMode === 'hierarchy'
                    ? "bg-white text-[var(--accent)] shadow-sm dark:bg-white/10"
                    : "text-[var(--text-muted)] hover:text-[var(--text-strong)]"
                )}
                onClick={() => setCareerMapMode('hierarchy')}
                title="Domains → Companies → Challenges"
              >
                Ecosystem
              </button>
              <button
                ...
                className={cn(
                  "px-3 py-2 text-sm font-semibold rounded-xl transition",
                  careerMapMode === 'taxonomy'
                    ? "bg-white text-[var(--accent)] shadow-sm dark:bg-white/10"
                    : "text-[var(--text-muted)] hover:text-[var(--text-strong)]"
                )}
                onClick={() => setCareerMapMode('taxonomy')}
                title="Role-family neighborhood graph"
              >
                Neighborhood
              </button>
            </div>
            ...
            {!isLgViewport ? (
              <button
                type="button"
                className="app-button-secondary !px-3 !py-2 bg-white/75 dark:bg-white/10"
          ...
          <div className="absolute inset-x-0 bottom-0 top-[var(--app-header-height)]">
            <CareerMap
              className="h-full !rounded-none"
              jobs={careerMapJobPool}
              selectedJobId={mapSelectedJobId}
              graphData={careerMapGraph}
              loading={careerMapLoading}
              error={careerMapError}
              mode={careerMapMode}
              centerNode={careerMapCenterNode}
              onSelectJob={(id: string | null) => {
                setMapSelectedJobId(id);
                if (id) setIsDetailPanelOpen(true);
              }}
            />
          </div>
        </div>
        ...
        <RoleDetailPanel
          job={careerMapJobPool.find((j) => j.id === mapSelectedJobId) || null}
          ...
          isSaved={mapSelectedJobId ? savedJobIds.includes(mapSelectedJobId) : false}
          userProfile={userProfile}
          variant="modal"
        />
      </section>
  ...
  return (
    <section style={forestAccentStyle} className="relative space-y-5 lg:pr-24">
      <ForestBackdrop />
      {setupSignals.length > 0 ? (
        <div className="space-y-0">
          <div id="challenge-why-roles">
            <SurfaceCard className="space-y-3 lg:rounded-b-none lg:border-b-0 lg:pb-0">
                ...
                <div>
                  ...
                  <p className="text-sm leading-6 text-[var(--text-muted)]">
                    {getLocaleLabel({
                      ...
                      pl: 'Krótki przegląd sygnałów, které w tej chwili najmocniej kształtují feed.',
      ...
      <PublicActivityPanel mode="discovery" className="border-[rgba(var(--accent-green-rgb),0.18)] bg-[linear-gradient(135deg,rgba(255,255,255,0.9),rgba(247,251,247,0.92))]" />
      ...
      <Toolbar sticky className="xl:hidden">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 p-1 rounded-2xl bg-[var(--surface-muted)] border border-[var(--border-subtle)]">
            <button
              onClick={() => setViewMode('list')}
              className="p-2 rounded-xl transition-all bg-white shadow-sm text-[var(--accent)]"
              title={copy.viewModeList}
              type="button"
            >
              <Layout size={18} />
            </button>
            <button
              ...
              className="p-2 rounded-xl transition-all text-[var(--text-faint)] hover:text-[var(--text-muted)]"
              title={copy.viewModeMap}
              type="button"
            >
              <Globe size={18} />
            </button>
      ...
      <React.Fragment>
        {/* filters moved into modal */}
              ...
              <div className="grid gap-4 p-4 lg:grid-cols-[minmax(0,1.2fr)_360px] lg:p-5">
                <div className="rounded-[var(--radius-xl)] border border-[var(--border-subtle)] bg-[var(--surface)] p-5 shadow-[var(--shadow-soft)]">
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <h3 className="text-xl font-semibold tracking-[-0.035em] text-[var(--text-strong)]">
                        {{
                    ...
                    <div className="flex flex-col gap-3 pt-1 sm:flex-row sm:items-center sm:justify-between">
                      {!hasPremiumAccess ? (
                        ...
                        <div className="rounded-[var(--radius-lg)] border border-amber-200/70 bg-amber-50 px-3 py-2.5 text-sm font-medium text-amber-700 dark:border-amber-900/70 dark:bg-amber-950/40 dark:text-amber-300">
          ...
          <div className={cn(mobileViewMode === 'swipe' && 'hidden xl:block')}>
            <SurfaceCard className="app-path-glow space-y-4 border-[rgba(var(--accent-green-rgb),0.18)] bg-[linear-gradient(135deg,rgba(255,252,245,0.98),rgba(246,251,247,0.98)_46%,rgba(243,249,255,0.98))] shadow-[0_24px_70px_-52px_rgba(15,23,42,0.3)] dark:bg-[linear-gradient(135deg,rgba(41,30,8,0.24),rgba(13,32,30,0.8)_48%,rgba(17,24,39,0.98))]">
            ...
            {loading && feedJobs.length === 0 ? (
              ...
              <div className="space-y-4">
                    ...
                    <SurfaceCard className="space-y-4">
                      ...
                      <div className="space-y-3">
                        {section.jobs.map((job) => {
                          ...
                          const isSelected = effectiveSelectedJobId === job.id;
                          ...
                          return (
                            <article
                              ...
                              onClick={() => {
                                if (isLgViewport) {
                                  setDesktopSelectedJobId(job.id);
                                  return;
                                }
                              ...
                              tabIndex={0}
                              className={cn(
                                'app-surface app-path-glow w-full rounded-[var(--radius-xl)] border p-5 text-left shadow-[var(--shadow-soft)] transition hover:-translate-y-[1px] hover:shadow-[var(--shadow-card)]',
                                ...
                                isSelected && 'border-[rgba(var(--accent-green-rgb),0.28)] bg-[linear-gradient(135deg,rgba(255,248,236,0.94),rgba(244,250,246,0.94)_56%,rgba(243,248,255,0.94))]'
                              )}
                            >
                              <div className="space-y-4">
                                <div className="flex flex-wrap items-start justify-between gap-4">
                                ...
                                <div className="space-y-4">
                                  <div>
                                    ...
                                    <h4 className="mt-2 max-w-3xl text-xl font-semibold leading-snug tracking-[-0.035em] text-[var(--text-strong)] md:text-[1.5rem]">
                                      {getChallengePreview(job)}
                                    </h4>
                                    <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-[var(--text-muted)]">
                                      <span className="font-semibold text-[var(--text-strong)]">{job.title}</span>
                                ...
                                <div className="flex flex-wrap gap-2">
                                  <MetaBadge tone="accent">{job.location || (isCsLike ? 'Lokalita TBD' : 'Location TBD')}</MetaBadge>
                                  <MetaBadge tone="accent">{formatSalary(job, i18n.language, isCsLike)}</MetaBadge>
                                  ...
                                  <MetaBadge>{copy.fit} {Math.round(job.jhi?.score || 0)}</MetaBadge>
                                </div>
                ...
                {hasMore ? (
                  <div className="flex justify-center border-t border-[var(--border-subtle)] pt-2">
                    <button
                      type="button"
                      className="app-button-secondary"
                    ...
                    >
                      {loadingMore
                        ? getLocaleLabel({
                          cs: 'Načítám další pozice…',
                          sk: 'Načítavam ďalšie pozície…',
                          de: 'Weitere Positionen werden geladen…',
                          at: 'Weitere Positionen werden geladen…',
                          pl: 'Ładowanie kolejnych ofert…',
                          en: 'Loading more roles…'
                        }, language)
                        : getLocaleLabel({
                          cs: 'Načíst další pozice',
                          sk: 'Načítať ďalšie pozície',
                          de: 'Weitere Positionen laden',
                          at: 'Weitere Positionen laden',
                          pl: 'Załaduj więcej ofert',
                          en: 'Load more roles'
                        }, language)}
                    </button>
                  </div>
                ) : null}
                <div ref={listLoadMoreSentinelRef} aria-hidden="true" className="h-2 w-full" />
          ...
          <div className="hidden lg:block lg:sticky lg:top-[calc(var(--app-toolbar-offset)+6px)] lg:self-start lg:max-h-[calc(100dvh-var(--app-toolbar-offset)-1.25rem)] lg:overflow-y-auto lg:pl-1">
            {selectedFeedJob ? (
              <RoleDetailPanel
                job={selectedFeedJob}
                isOpen={true}
                onClose={() => setDesktopSelectedJobId(null)}
                onOpenProfile={onOpenProfile}
                onToggleSave={handleToggleSave}
                isSaved={savedJobIds.includes(selectedFeedJob.id)}
                userProfile={userProfile}
                variant="docked"
              />
            ) : (
              <SurfaceCard className="space-y-3 border-[rgba(var(--accent-rgb),0.14)] bg-white/80 p-5 shadow-[var(--shadow-overlay)] dark:bg-[rgba(15,23,42,0.78)]">
                <div className="text-sm font-semibold text-[var(--text-strong)]">
                  {isCsLike ? 'Vyberte nabídku' : 'Select a role'}
                </div>
                <p className="text-sm leading-6 text-[var(--text-muted)]">
                  {isCsLike ? 'Klikněte na kartu ve feedu a detail se zobrazí zde.' : 'Click a card in the feed to see details here.'}
                </p>
              </SurfaceCard>
            )}
          </div>
      ...
      {!isLgViewport ? (
        <RoleDetailPanel
          ...
          onOpenProfile={onOpenProfile}
          onToggleSave={handleToggleSave}
          isSaved={selectedFeedJob ? savedJobIds.includes(selectedFeedJob.id) : false}
          userProfile={userProfile}
          variant="side"
        />
      ) : null}
...
const MetaBadge: React.FC<{ children: React.ReactNode; tone?: 'default' | 'accent' }> = ({ children, tone = 'default' }) => (
  <span
    className={cn(
      ...
      tone === 'accent'
        ? 'bg-[linear-gradient(135deg,rgba(var(--accent-rgb),0.14),rgba(var(--accent-green-rgb),0.1))] text-[var(--accent)]'
        : 'bg-[rgba(255,255,255,0.7)] text-[var(--text-muted)]'
    )}
  >
    {children}
  </span>
