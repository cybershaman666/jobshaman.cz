...
const MyInvitations: React.FC<{ forCompany?: boolean }> = ({ forCompany = false }) => {
  ...
  const handleStartAssessment = async (invitation: Invitation) => {
    ...
    try {
      ...
      alert("Nepodařilo se načíst test. Zkuste to prosím později.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAssessmentComplete = (_resultId: string) => {
    ...
    alert("Test byl úspěšně odeslán! Děkujeme.");
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <h4 className="font-bold text-slate-900 dark:text-white">Pozvánky k Testům</h4>
        <button onClick={load} className="text-sm text-slate-500 hover:text-cyan-600 transition-colors">Obnovit</button>
      </div>

      {loading && <div className="text-sm text-slate-500 animate-pulse">Načítám data...</div>}
      ...
      {!loading && !invitations.length && (
        <div className="text-center p-8 bg-slate-50 dark:bg-slate-900 rounded-xl border border-dashed border-slate-300 dark:border-slate-700">
          <p className="text-slate-500">Zatím nemáte žádné aktivní pozvánky.</p>
        </div>
      )}
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                <div className={`text-sm font-bold px-3 py-1 rounded-full ${inv.status === 'completed' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' :
                  ...
                  {inv.status === 'pending' ? 'Čeká na vypracování' :
                    inv.status === 'completed' ? 'Dokončeno' : 'Vypršelo'}
                </div>
