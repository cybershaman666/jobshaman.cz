...
const PRODUCT_DATA_ACCESS_ERROR = 'V2 frontend Supabase client is auth-only. Product data must go through /api/v2.';

// Configuration
const supabaseUrl = import.meta.env.VITE_SUPABASE_URL || import.meta.env.SUPABASE_URL || '';
const supabaseKey = import.meta.env.VITE_SUPABASE_ANON_KEY || import.meta.env.VITE_SUPABASE_KEY || import.meta.env.SUPABASE_KEY || '';
...
export const getSupabaseClient = () => {
    ...
    try {
        ...
        const rawSupabaseClient = createClient(supabaseUrl, supabaseKey, {
            auth: {
                persistSession: true,
        ...
        rawSupabaseClient.auth.onAuthStateChange((event: any, _session: any) => {
        ...
        supabaseInstance = new Proxy(rawSupabaseClient, {
            get(target, property, receiver) {
                if (property === 'from' || property === 'rpc' || property === 'storage' || property === 'functions' || property === 'realtime') {
                    throw new Error(PRODUCT_DATA_ACCESS_ERROR);
                }
