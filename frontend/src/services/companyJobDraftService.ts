...
const normalizeRoleDraft = (row: any): JobDraft => ({
    ...
    handshake_blueprint_v1:
        (row?.editor_state && typeof row.editor_state === 'object' && (row.editor_state as any).handshake_blueprint_v1 && typeof (row.editor_state as any).handshake_blueprint_v1 === 'object')
            ? (row.editor_state as any).handshake_blueprint_v1
            : row?.handshake_blueprint_v1 ?? null,
}) as JobDraft;
