...
export const fetchJcfpmItems = async (): Promise<JcfpmItem[]> => {
  ...
  const fetchFromBackendPublic = async () => {
    ...
    try {
      const response = await withTimeout(authenticatedFetch(`${BACKEND_URL}/candidate/jcfpm/items`, {
        method: 'GET',
  ...
  try {
    ...
    if (retryable) {
      ...
      try {
        ...
        // Fall through to Supabase compatibility fallback below.
      }
    }
...
export const submitJcfpm = async (
  ...
  try {
    const response = await withTimeout(
      authenticatedFetch(`${BACKEND_URL}/candidate/jcfpm/submit`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          responses,
