...
export async function getAdminMe() {
  const response = await authenticatedFetch(`${BACKEND_URL}/admin/me`, {
    method: 'GET',
  ...
  if (!response.ok) {
    ...
    throw new AdminRequestError(error.detail || 'Failed to load admin identity', response.status);
  }
...
export async function getAdminSubscriptions(filters: AdminSubscriptionFilters = {}) {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to load subscriptions', response.status);
  }
...
export async function updateAdminSubscription(payload: AdminSubscriptionUpdate) {
  ...
  if (!response.ok) {
    ...
    throw new AdminRequestError(`Failed to update subscription: ${detail}`, response.status);
  }
...
export async function getAdminNotifications(daysAhead: number = 7) {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to load notifications', response.status);
  }
...
export async function getAdminSubscriptionAudit(subscriptionId: string, limit: number = 50) {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to load audit log', response.status);
  }
...
export async function getAdminCrmEntityDetail(kind: 'company' | 'user' | 'lead', entityId: string) {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to load CRM entity detail', response.status);
  }
...
export async function getAdminCrmEntities(params: { q?: string; kind?: 'company' | 'user' | 'all'; limit?: number } = {}) {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to load CRM entities', response.status);
  }
...
export async function getAdminCrmJobReactionsSummary(params: { q?: string; limit?: number; window_days?: number } = {}) {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to load CRM job reaction summary', response.status);
  }
...
export async function getAdminCrmLeads(params: { q?: string; status?: string; limit?: number; offset?: number } = {}) {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to load CRM leads', response.status);
  }
...
export async function createAdminCrmLead(payload: AdminCrmLeadPayload) {
  ...
  if (response.status === 404) {
    throw new AdminRequestError('CRM leads support is not deployed on the current backend yet', response.status);
  }

  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to create CRM lead', response.status);
  }
...
export async function updateAdminCrmLead(leadId: string, payload: Partial<AdminCrmLeadPayload>) {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to update CRM lead', response.status);
  }
...
export async function getAdminFounderBoard(params: { q?: string; status?: string; limit?: number } = {}) {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to load founder board', response.status);
  }
...
export async function createAdminFounderBoardCard(payload: AdminFounderBoardCardPayload) {
  ...
  if (response.status === 404) {
    throw new AdminRequestError('Founder board support is not deployed on the current backend yet', response.status);
  }

  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to create founder board card', response.status);
  }
...
export async function updateAdminFounderBoardCard(cardId: string, payload: Partial<AdminFounderBoardCardPayload>) {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to update founder board card', response.status);
  }
...
export async function createAdminFounderBoardComment(cardId: string, payload: AdminFounderBoardCommentPayload) {
  ...
  if (response.status === 404) {
    throw new AdminRequestError('Founder board comments are not deployed on the current backend yet', response.status);
  }

  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to create founder board comment', response.status);
  }
...
export async function getAdminStats() {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to load stats', response.status);
  }
...
export async function getAdminAiQuality(days: number = 30) {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to load AI quality stats', response.status);
  }
...
export async function adminSearch(query: string, kind: 'company' | 'user' | 'lead') {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Search failed', response.status);
  }
...
export async function getAdminPushSubscriptions(params: { q?: string; limit?: number; offset?: number; activeOnly?: boolean } = {}) {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to load push subscriptions', response.status);
  }
...
export async function getAdminUserDigest(userId: string) {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to load user digest settings', response.status);
  }
...
export async function updateAdminUserDigest(userId: string, payload: {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    const detail = error.detail || error.message || `HTTP ${response.status}`;
    throw new AdminRequestError(`Failed to update user digest: ${detail}`, response.status);
  }
...
export async function getAdminJobRoles(params: { q?: string; limit?: number; offset?: number } = {}) {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    throw new AdminRequestError(error.detail || 'Failed to load job roles', response.status);
  }
...
export async function createAdminJobRole(payload: {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    const detail = error.detail || error.message || `HTTP ${response.status}`;
    throw new AdminRequestError(`Failed to create job role: ${detail}`, response.status);
  }
...
export async function updateAdminJobRole(roleId: string, payload: {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    const detail = error.detail || error.message || `HTTP ${response.status}`;
    throw new AdminRequestError(`Failed to update job role: ${detail}`, response.status);
  }
...
export async function deleteAdminJobRole(roleId: string) {
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    const detail = error.detail || error.message || `HTTP ${response.status}`;
    throw new AdminRequestError(`Failed to delete job role: ${detail}`, response.status);
  }
