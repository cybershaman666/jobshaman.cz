  ...
  return (
    <StepContainer step={step} stepIndex={stepIndex} totalSteps={totalSteps}>
      <div className="space-y-6">
        {/* Prompt */}
        <div className="rounded-[12px] border border-purple-200 bg-purple-50 p-6">
          <div className="text-[11px] font-bold uppercase text-purple-700 tracking-wide">
            {t('rebuild.journey.schedule_prompt', { defaultValue: 'Next Steps' })}
          </div>
          <p className="mt-3 text-base leading-7 text-purple-900">
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isSelected
                        ? 'border-[#1f5fbf] bg-[#dbeafe] shadow-md'
                  ...
                  >
                    <div className="flex gap-3">
                      <div className={cn(
                        ...
                        isSelected ? 'bg-[#1f5fbf] text-white' : 'bg-slate-100 text-slate-600'
                      )}>
                        <Calendar size={18} />
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className={cn(
                          'font-semibold',
                          isSelected ? 'text-[#1f5fbf]' : 'text-slate-900'
                        )}>
                          {formatSlotTime(slot)}
                        </div>
                        <div className={cn(
                          'text-sm mt-1 flex items-center gap-1',
                          isSelected ? 'text-[#0f4a8f]' : 'text-slate-600'
                        )}>
                          <Clock size={14} />
                          {slot.duration} min
                        </div>
                        <div className={cn(
                          'text-xs mt-1',
                          isSelected ? 'text-[#0f4a8f]' : 'text-slate-500'
                        )}>
                          {slot.timezone}
                        </div>
                      </div>
                      {isSelected && (
                        <div className="flex-shrink-0 text-[#1f5fbf] font-bold text-xl">
                          ✓
                        </div>
                      )}
                    </div>
