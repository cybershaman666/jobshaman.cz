...
export const CandidateJcfpmPage: React.FC<{
  ...
  const dimensionReport = React.useMemo<Record<string, { summary: string; high: string; balanced: string; low: string; development: string; environment: string }>>(() => ({
    d1_cognitive: {
      summary: t('rebuild.jcfpm.report.d1_cognitive.summary', { defaultValue: 'Způsob zpracování informací a řešení problémů.' }),
      high: t('rebuild.jcfpm.report.d1_cognitive.high', { defaultValue: 'Přirozeně si rovnáš fakta, souvislosti a dopady. Vynikáš tam, kde je potřeba oddělit šum od podstaty.' }),
      balanced: t('rebuild.jcfpm.report.d1_cognitive.balanced', { defaultValue: 'Umíš kombinovat analýzu s praktickou intuicí. Nejlépe funguješ, když máš dost signálů, ale nezasekneš se v detailu.' }),
      low: t('rebuild.jcfpm.report.d1_cognitive.low', { defaultValue: 'Více se opíráš o intuici a celkový dojem. U důležitých rozhodnutí pomůže krátká kontrola faktů.' }),
      development: t('rebuild.jcfpm.report.d1_cognitive.development', { defaultValue: 'Před důležitým rozhodnutím si sepiš 3 fakta, 1 riziko a 1 alternativu.' }),
      environment: t('rebuild.jcfpm.report.d1_cognitive.environment', { defaultValue: 'Role s jasným kontextem, možností analyzovat problém a převádět ho do rozhodnutí.' }),
    },
    d2_social: {
      summary: t('rebuild.jcfpm.report.d2_social.summary', { defaultValue: 'Preferovaný způsob interakce s lidmi a týmové dynamiky.' }),
      high: t('rebuild.jcfpm.report.d2_social.high', { defaultValue: 'Umíš držet důvěru, domluvu a společný směr. Tvoje síla je číst lidi bez ztráty věcnosti.' }),
      balanced: t('rebuild.jcfpm.report.d2_social.balanced', { defaultValue: 'Sociální orientace je vyvážená. Dokážeš fungovat v týmu i samostatně podle potřeby.' }),
      low: t('rebuild.jcfpm.report.d2_social.low', { defaultValue: 'Pravděpodobně preferuješ samostatnou práci a menší množství sociální koordinace.' }),
      development: t('rebuild.jcfpm.report.d2_social.development', { defaultValue: 'Před schůzkou si pojmenuj očekávaný výsledek a po ní jeden jasný další krok.' }),
      environment: t('rebuild.jcfpm.report.d2_social.environment', { defaultValue: 'Tým se srozumitelnou komunikací, pravidelnou zpětnou vazbou a rozumnou mírou autonomie.' }),
    },
    d3_motivational: {
      summary: t('rebuild.jcfpm.report.d3_motivational.summary', { defaultValue: 'Co tě pohání a co považuješ za odměnu.' }),
      high: t('rebuild.jcfpm.report.d3_motivational.high', { defaultValue: 'Silně tě táhne smysl, dopad a růst odpovědnosti. Potřebuješ chápat proč, ne jen co.' }),
      balanced: t('rebuild.jcfpm.report.d3_motivational.balanced', { defaultValue: 'Zvládáš jak vnitřní smysl, tak jasné cíle a odměny. Pomáhá ujasnit si aktuální prioritu.' }),
      low: t('rebuild.jcfpm.report.d3_motivational.low', { defaultValue: 'Motivace může víc stát na konkrétní odměně, stabilitě nebo jasně zadaném výsledku.' }),
      development: t('rebuild.jcfpm.report.d3_motivational.development', { defaultValue: 'U každého většího úkolu si napiš, komu pomáhá a podle čeho poznáš dobrý výsledek.' }),
      environment: t('rebuild.jcfpm.report.d3_motivational.environment', { defaultValue: 'Projekty s viditelným dopadem, férovou odpovědností a prostorem něco zlepšovat.' }),
    },
    d4_energy: {
      summary: t('rebuild.jcfpm.report.d4_energy.summary', { defaultValue: 'Tempo, intenzita a styl práce.' }),
      high: t('rebuild.jcfpm.report.d4_energy.high', { defaultValue: 'Zvládáš vyšší tempo a dynamické situace. Klíčové je plánovat regeneraci, aby výkon zůstal udržitelný.' }),
      balanced: t('rebuild.jcfpm.report.d4_energy.balanced', { defaultValue: 'Umíš přepínat mezi sprintem a stabilním tempem. Dobře funguje rytmus s jasnými prioritami.' }),
      low: t('rebuild.jcfpm.report.d4_energy.low', { defaultValue: 'Spíš potřebuješ stabilnější tempo, méně přepínání a chráněný prostor pro soustředění.' }),
      development: t('rebuild.jcfpm.report.d4_energy.development', { defaultValue: 'Na 14 dní si nastav rytmus sprint, pauza, review: co bere energii a co ji vrací.' }),
      environment: t('rebuild.jcfpm.report.d4_energy.environment', { defaultValue: 'Práce s realistickými prioritami, možností chránit hlubokou práci a domluvenými sprinty.' }),
    },
    d5_values: {
      summary: t('rebuild.jcfpm.report.d5_values.summary', { defaultValue: 'Co musí práce přinášet, aby dávala smysl.' }),
      high: t('rebuild.jcfpm.report.d5_values.high', { defaultValue: 'Hodnoty jsou silná kotva. Integrita, důvěra a dlouhodobý dopad pro tebe nejsou dekorace.' }),
      balanced: t('rebuild.jcfpm.report.d5_values.balanced', { defaultValue: 'Hodnoty máš vyvážené a umíš se přizpůsobit různým kulturám, pokud víš, co je důležité.' }),
      low: t('rebuild.jcfpm.report.d5_values.low', { defaultValue: 'Můžeš snáze tolerovat různá prostředí, ale vyplatí se vědomě pojmenovat vlastní hranice.' }),
      development: t('rebuild.jcfpm.report.d5_values.development', { defaultValue: 'Sepiš si 3 pracovní situace, které jsou pro tebe nepřekročitelné, a proč.' }),
      environment: t('rebuild.jcfpm.report.d5_values.environment', { defaultValue: 'Kultura s férovostí, transparentností a prostorem mluvit o dopadech rozhodnutí.' }),
    },
    d6_ai_readiness: {
      summary: t('rebuild.jcfpm.report.d6_ai_readiness.summary', { defaultValue: 'Jak dobře prosperuješ v rychle se měnícím technologickém prostředí.' }),
      high: t('rebuild.jcfpm.report.d6_ai_readiness.high', { defaultValue: 'Technologická adaptabilita je silná stránka. Rychle testuješ nové nástroje a hledáš praktickou hodnotu.' }),
      balanced: t('rebuild.jcfpm.report.d6_ai_readiness.balanced', { defaultValue: 'Nové nástroje umíš použít, když dávají smysl. Nejlépe funguje praktické učení na reálném úkolu.' }),
      low: t('rebuild.jcfpm.report.d6_ai_readiness.low', { defaultValue: 'Změna nástrojů může stát víc energie. Pomůže bezpečný trénink na malých pracovních případech.' }),
      development: t('rebuild.jcfpm.report.d6_ai_readiness.development', { defaultValue: 'Vyber jeden opakovaný úkol a zkus na něm AI použít třikrát s krátkým vyhodnocením.' }),
      environment: t('rebuild.jcfpm.report.d6_ai_readiness.environment', { defaultValue: 'Prostředí, kde se nové nástroje používají běžně, ale s jasným lidským úsudkem.' }),
    },
    d7_cognitive_reflection: {
      summary: t('rebuild.jcfpm.report.d7_cognitive_reflection.summary', { defaultValue: 'Schopnost zastavit první intuici a ověřit ji logikou.' }),
      high: t('rebuild.jcfpm.report.d7_cognitive_reflection.high', { defaultValue: 'Umíš brzdit první závěr a ověřovat důkazy. To zvyšuje kvalitu rozhodnutí s vyšším dopadem.' }),
      balanced: t('rebuild.jcfpm.report.d7_cognitive_reflection.balanced', { defaultValue: 'Intuici umíš doplnit kontrolou. Pomáhá jednoduchý rozhodovací checklist u větších kroků.' }),
      low: t('rebuild.jcfpm.report.d7_cognitive_reflection.low', { defaultValue: 'Rychlá intuice může dominovat. U důležitých věcí přidej krátké zastav-se-ověř.' }),
      development: t('rebuild.jcfpm.report.d7_cognitive_reflection.development', { defaultValue: 'Před rozhodnutím si polož otázku: Jaký je důkaz a co by mě přesvědčilo o opaku?' }),
      environment: t('rebuild.jcfpm.report.d7_cognitive_reflection.environment', { defaultValue: 'Role, kde je čas na review rozhodnutí a chyby se používají jako učení, ne jako trest.' }),
    },
    d8_digital_eq: {
      summary: t('rebuild.jcfpm.report.d8_digital_eq.summary', { defaultValue: 'Citlivost na emoce, tón a důvěru v textové komunikaci.' }),
      high: t('rebuild.jcfpm.report.d8_digital_eq.high', { defaultValue: 'V textu dobře čteš tón, podtext i důvěru. To je silné v asynchronní spolupráci a citlivých tématech.' }),
      balanced: t('rebuild.jcfpm.report.d8_digital_eq.balanced', { defaultValue: 'Digitální komunikaci zvládáš prakticky. Pomáhá vědomě volit mezi zprávou a hovorem.' }),
      low: t('rebuild.jcfpm.report.d8_digital_eq.low', { defaultValue: 'V textu může snáz vzniknout nedorozumění. Pomůže kratší zpráva, jasný záměr a ověření dopadu.' }),
      development: t('rebuild.jcfpm.report.d8_digital_eq.development', { defaultValue: 'U náročné zprávy napiš nejdřív záměr, potom fakta a nakonec konkrétní prosbu.' }),
      environment: t('rebuild.jcfpm.report.d8_digital_eq.environment', { defaultValue: 'Hybridní nebo remote tým s jasnou komunikační hygienou a bezpečným prostorem pro otázky.' }),
    },
    d9_systems_thinking: {
      summary: t('rebuild.jcfpm.report.d9_systems_thinking.summary', { defaultValue: 'Jak dobře čteš vztahy, zpětné vazby a vedlejší efekty.' }),
      high: t('rebuild.jcfpm.report.d9_systems_thinking.high', { defaultValue: 'Silné systémové myšlení. Vidíš vztahy, zpožděné dopady a místa, kde se problém vrací.' }),
      balanced: t('rebuild.jcfpm.report.d9_systems_thinking.balanced', { defaultValue: 'Umíš sledovat širší souvislosti, když máš jasně vymezený rámec problému.' }),
      low: t('rebuild.jcfpm.report.d9_systems_thinking.low', { defaultValue: 'Může být užitečné vědomě mapovat příčiny, následky a závislosti místo řešení izolovaného symptomu.' }),
      development: t('rebuild.jcfpm.report.d9_systems_thinking.development', { defaultValue: 'U jednoho problému nakresli mapu: příčina, dopad, zpětná vazba, skrytý náklad.' }),
      environment: t('rebuild.jcfpm.report.d9_systems_thinking.environment', { defaultValue: 'Procesní, produktové nebo provozní role, kde se zlepšují celé systémy, ne jen jednotlivé úkoly.' }),
    },
    d10_ambiguity_interpretation: {
      summary: t('rebuild.jcfpm.report.d10_ambiguity_interpretation.summary', { defaultValue: 'Jak čteš nejasné situace: rizika vs. příležitosti.' }),
      high: t('rebuild.jcfpm.report.d10_ambiguity_interpretation.high', { defaultValue: 'V nejistotě vidíš směr a příležitosti. Umíš začít, i když zadání ještě není dokonalé.' }),
      balanced: t('rebuild.jcfpm.report.d10_ambiguity_interpretation.balanced', { defaultValue: 'Nejasnost zvládáš, pokud máš možnost postupně ověřovat další kroky.' }),
      low: t('rebuild.jcfpm.report.d10_ambiguity_interpretation.low', { defaultValue: 'Nejasné zadání může brzdit energii. Pomůže definovat minimální další signál a první bezpečný krok.' }),
      development: t('rebuild.jcfpm.report.d10_ambiguity_interpretation.development', { defaultValue: 'U nejasného úkolu si napiš: co vím, co nevím, co ověřím do 48 hodin.' }),
      environment: t('rebuild.jcfpm.report.d10_ambiguity_interpretation.environment', { defaultValue: 'Prostředí s proměnlivými zadáními, ale dobrou oporou v prioritách a rozhodovacích právech.' }),
    },
    d11_problem_decomposition: {
      summary: t('rebuild.jcfpm.report.d11_problem_decomposition.summary', { defaultValue: 'Schopnost rozsekat velký problém na jasné kroky.' }),
      high: t('rebuild.jcfpm.report.d11_problem_decomposition.high', { defaultValue: 'Umíš rychle vytvořit strukturu kroků. Silné pro strategii, delivery, provozní změny i komplexní projekty.' }),
      balanced: t('rebuild.jcfpm.report.d11_problem_decomposition.balanced', { defaultValue: 'Dokážeš problém rozložit, když máš jasný cíl a dost kontextu.' }),
      low: t('rebuild.jcfpm.report.d11_problem_decomposition.low', { defaultValue: 'Velký problém může působit zahlcujícím dojmem. Pomůže jeden malý experiment a jednoduché priority.' }),
      development: t('rebuild.jcfpm.report.d11_problem_decomposition.development', { defaultValue: 'Každý větší problém rozděl na blokátor, první krok a měřitelný výstup týdne.' }),
      environment: t('rebuild.jcfpm.report.d11_problem_decomposition.environment', { defaultValue: 'Role, kde se strategie převádí do priorit, experimentů a praktického doručení.' }),
    },
    d12_moral_compass: {
      summary: t('rebuild.jcfpm.report.d12_moral_compass.summary', { defaultValue: 'Stabilita hodnot v dilematech a tlakových situacích.' }),
      high: t('rebuild.jcfpm.report.d12_moral_compass.high', { defaultValue: 'Silný morální kompas. V tlaku držíš integritu, důvěru a dlouhodobé dopady.' }),
      balanced: t('rebuild.jcfpm.report.d12_moral_compass.balanced', { defaultValue: 'Etické dopady bereš v úvahu, zejména když jsou jasně pojmenované.' }),
      low: t('rebuild.jcfpm.report.d12_moral_compass.low', { defaultValue: 'V tlaku může být těžší otevřít nepohodlné téma. Pomůže předem daný princip rozhodování.' }),
      development: t('rebuild.jcfpm.report.d12_moral_compass.development', { defaultValue: 'U dilemat si napiš: koho to ovlivní, co se stane později a co bych obhájil/a nahlas.' }),
      environment: t('rebuild.jcfpm.report.d12_moral_compass.environment', { defaultValue: 'Organizace, která bere vážně důvěru, odpovědnost a transparentní rozhodování.' }),
    },
    i1_love: {
      summary: t('rebuild.jcfpm.report.i1_love.summary', { defaultValue: 'Co tě přirozeně vtahuje a nabíjí.' }),
      high: t('rebuild.jcfpm.report.i1_love.high', { defaultValue: 'Máš silný signál vnitřního zájmu. Hledej role, kde se tato energie používá často, ne jen okrajově.' }),
      balanced: t('rebuild.jcfpm.report.i1_love.balanced', { defaultValue: 'Zájem se probouzí v konkrétních situacích. Vyplatí se sledovat, kdy energie roste.' }),
      low: t('rebuild.jcfpm.report.i1_love.low', { defaultValue: 'Zatím není jasné, co tě pracovně opravdu vtahuje. Pomůže mapovat aktivity, u kterých mizí čas.' }),
      development: t('rebuild.jcfpm.report.i1_love.development', { defaultValue: 'Týden si zapisuj 3 momenty, kdy práce energii přidala nebo vzala.' }),
      environment: t('rebuild.jcfpm.report.i1_love.environment', { defaultValue: 'Práce s prostorem pro ponor, tvorbu nebo problém, který tě osobně zajímá.' }),
    },
    i2_good_at: {
      summary: t('rebuild.jcfpm.report.i2_good_at.summary', { defaultValue: 'V čem máš opakovaně použitelnou sílu.' }),
      high: t('rebuild.jcfpm.report.i2_good_at.high', { defaultValue: 'Silně vnímáš své schopnosti a umíš je převést do přínosu pro ostatní.' }),
      balanced: t('rebuild.jcfpm.report.i2_good_at.balanced', { defaultValue: 'Některé silné stránky už znáš, další se ukážou přes konkrétní pracovní důkazy.' }),
      low: t('rebuild.jcfpm.report.i2_good_at.low', { defaultValue: 'Můžeš podceňovat, v čem jsi dobrý/á. Hledej opakované žádosti o pomoc a výsledky.' }),
      development: t('rebuild.jcfpm.report.i2_good_at.development', { defaultValue: 'Sepiš 5 situací, kdy jsi někomu pomohl/a, a najdi společnou schopnost.' }),
      environment: t('rebuild.jcfpm.report.i2_good_at.environment', { defaultValue: 'Role, kde se tvoje silné schopnosti používají pravidelně a jsou viditelné ve výsledku.' }),
    },
    i3_world_needs: {
      summary: t('rebuild.jcfpm.report.i3_world_needs.summary', { defaultValue: 'Jak moc potřebuješ vidět užitek pro lidi nebo svět.' }),
      high: t('rebuild.jcfpm.report.i3_world_needs.high', { defaultValue: 'Dopad na konkrétní lidi je pro tebe důležitý zdroj smyslu a výdrže.' }),
      balanced: t('rebuild.jcfpm.report.i3_world_needs.balanced', { defaultValue: 'Smysl a užitek jsou důležité, ale nemusí být jediným zdrojem motivace.' }),
      low: t('rebuild.jcfpm.report.i3_world_needs.low', { defaultValue: 'Práce může dávat smysl i bez výrazného společenského dopadu, pokud sedí role, tým nebo odměna.' }),
      development: t('rebuild.jcfpm.report.i3_world_needs.development', { defaultValue: 'U nabídky práce si ověř, komu výsledek pomáhá a jak se to pozná.' }),
      environment: t('rebuild.jcfpm.report.i3_world_needs.environment', { defaultValue: 'Produkty, služby nebo provozy, kde je vidět reálný užitek práce.' }),
    },
    i4_paid_for: {
      summary: t('rebuild.jcfpm.report.i4_paid_for.summary', { defaultValue: 'Jak propojuješ smysl, schopnosti a ekonomickou hodnotu.' }),
      high: t('rebuild.jcfpm.report.i4_paid_for.high', { defaultValue: 'Dobře přemýšlíš o hodnotě své práce pro trh. To pomáhá volit udržitelné směřování.' }),
      balanced: t('rebuild.jcfpm.report.i4_paid_for.balanced', { defaultValue: 'Ekonomickou hodnotu bereš v úvahu, ale bude dobré ji zpřesnit konkrétními důkazy.' }),
      low: t('rebuild.jcfpm.report.i4_paid_for.low', { defaultValue: 'Může být potřeba víc přeložit schopnosti do řeči hodnoty pro firmu nebo zákazníka.' }),
      development: t('rebuild.jcfpm.report.i4_paid_for.development', { defaultValue: 'Ke každé silné stránce napiš jeden měřitelný přínos pro tým, zákazníka nebo firmu.' }),
      environment: t('rebuild.jcfpm.report.i4_paid_for.environment', { defaultValue: 'Role, kde je jasné, jak tvoje práce vytváří hodnotu a férovou odměnu.' }),
    },
  }), [t]);
                        ...
                        {targets.map((target) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="mt-1 text-xs font-medium text-slate-500">{paired ? sources.find((source) => optionId(source) === paired.source)?.label || paired.source : t('rebuild.jcfpm.drag_assign_cause', { defaultValue: 'Sem přetáhni nebo přiřaď příčinu' })}</div>
                            </button>
                          );
                        })}
                      </div>
