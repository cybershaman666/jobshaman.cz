...
const backendEnv = { ...env, ...(python.env ?? {}) };
if (backendEnv.PYTHONPATH) {
  backendEnv.PYTHONPATH = `${path.join(rootDir, 'backend')}${path.delimiter}${backendEnv.PYTHONPATH}`;
} else {
  backendEnv.PYTHONPATH = path.join(rootDir, 'backend');
}
