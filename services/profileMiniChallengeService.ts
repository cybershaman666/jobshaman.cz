...
export const createProfileMiniChallenge = async (input: ProfileMiniChallengeCreateInput): Promise<Job> => {
  ...
  const jobs = await fetchJobsByIds([String(rawJob.id)]);
  const job = jobs[0];
  if (!job) {
    throw new Error('Published mini challenge could not be hydrated.');
  }
  return mergePublisherJobMeta([job], [rawJob])[0];
};
...
export const updateProfileMiniChallengeLifecycle = async (
  ...
  const response = await authenticatedFetch(`${BACKEND_URL}/publisher/mini-challenges/${jobId}/lifecycle`, {
    method: 'PATCH',
    headers: jsonHeaders,
...
export const fetchProfileMiniChallengeDialogues = async (jobId: string | number): Promise<CompanyApplicationRow[]> => {
  const response = await authenticatedFetch(`${BACKEND_URL}/publisher/mini-challenges/${jobId}/dialogues`, {
    method: 'GET',
    headers: jsonHeaders,
  });
  if (!response.ok) {
