-- Migration: Add job interactions + personalized re-ranking
-- Created: 2026-02-03

-- 1) New table for swipe/interactions
...
CREATE INDEX IF NOT EXISTS idx_job_interactions_user_time ON public.job_interactions (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_job_interactions_job_time ON public.job_interactions (job_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_job_interactions_user_job ON public.job_interactions (user_id, job_id);

-- 2) Re-create search function with personalization signals
