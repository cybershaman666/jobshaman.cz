-- Migration: tighten precision for very short search terms (e.g. "AI")
-- Created: 2026-02-16
-- Purpose: avoid broad matches where 1-2 char terms return unrelated titles.
...
BEGIN
    ...
    WITH params AS (
        SELECT
            btrim(COALESCE(search_term, '')) AS q_raw,
            btrim(regexp_replace(lower(public.unaccent(COALESCE(search_term, ''))), '[^[:alnum:]]+', ' ', 'g')) AS q_tokenized,
            char_length(replace(btrim(regexp_replace(lower(public.unaccent(COALESCE(search_term, ''))), '[^[:alnum:]]+', ' ', 'g')), ' ', '')) AS q_compact_len
    ),
    base AS (
        SELECT
            j.*,
        ...
        WHERE
            (
                ...
                (
                    ...
                    AND p.q_tokenized <> ''
                    AND strpos(
                        ' ' || regexp_replace(lower(public.unaccent(COALESCE(j.title, ''))), '[^[:alnum:]]+', ' ', 'g') || ' ',
                        ' ' || p.q_tokenized || ' '
                    ) > 0
                ) OR
                (
                    p.q_compact_len >= 3
                    AND (
                        public.unaccent(j.title) ILIKE '%' || public.unaccent(p.q_raw) || '%'
                        OR j.title % p.q_raw
                    )
                )
            )
    ...
    SELECT
        ...
        CASE
            ...
            ELSE calculate_job_relevance_score(j.title, NULL, search_term)
    ...
    ORDER BY
        (
            CASE
                ...
                THEN calculate_job_relevance_score(j.title, NULL, search_term)
                ELSE 0
            END
