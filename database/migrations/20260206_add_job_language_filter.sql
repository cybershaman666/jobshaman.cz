-- Migration: add language_code to jobs + language filter in search RPC
-- Created: 2026-02-06

-- 1) Add language_code column
ALTER TABLE public.jobs
  ADD COLUMN IF NOT EXISTS language_code text;

-- 2) Optional index for filtering
CREATE INDEX IF NOT EXISTS idx_jobs_language_code ON public.jobs (language_code);

-- 3) Re-create search function with language filter
...
-- Grant execute to roles (updated signature)
