-- JOB SHAMAN - COMPLETE DATABASE RECOVERY SCRIPT
...
-- 3. ENSURE COMPANY MEMBERS TABLE EXISTS (For recruiter authorization)
...
-- Ensure jobs table exists (if 404 was due to missing table)
...
-- Add missing company_id column to jobs
...
-- 5. CREATE SPATIAL INDEX AND TRIGGER
CREATE INDEX IF NOT EXISTS idx_jobs_geom ON public.jobs USING GIST (geom);
CREATE INDEX IF NOT EXISTS idx_jobs_company_id ON public.jobs(company_id);
CREATE OR REPLACE FUNCTION update_job_geom() RETURNS TRIGGER AS $$ BEGIN IF NEW.lat IS NOT NULL
...
INSERT
    OR
UPDATE ON public.jobs FOR EACH ROW EXECUTE FUNCTION update_job_geom();
...
-- Enable RLS and add public access
...
-- 8. FIX RLS FOR JOBS (Ensure companies can read/write their own jobs)
...
SELECT USING (true);
-- 9. DISABLE RLS ON COMPANY_MEMBERS (To avoid infinite recursion issues)
