...
def upsert_jobspy_documents(documents: list[dict[str, Any]]) -> dict[str, int]:
    ...
    with conn.cursor() as cur:
        ...
        cur.executemany(
            ...
            VALUES (%(id)s, %(provider)s, %(source_site)s, %(title)s, %(company)s, %(location)s, %(city)s, %(state)s, %(country)s, %(country_code)s, %(job_type)s, %(interval)s, %(min_amount)s, %(max_amount)s, %(currency)s, %(job_url)s, %(description)s, %(is_remote)s, %(lat)s, %(lng)s, %(geocode_source)s, %(search_term)s, %(google_search_term)s, %(location_query)s, %(search_location_query)s, %(hours_old)s, %(query_hash)s, %(search_blob)s, %(raw_payload_json)s::jsonb, %(payload_json)s::jsonb, %(scraped_at)s, %(expires_at)s, %(updated_at)s)
            ON CONFLICT (id) DO UPDATE SET
                provider = EXCLUDED.provider,
